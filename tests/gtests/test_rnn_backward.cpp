...
template <typename data_t>
void compute_ref_lstm_bwd(const test_lstm_desc_t &ld,
        ...
        const memory &dweights)
{
    ...
    size_t temp_size = btmp * batch_size + gates_space_size + (2 * hout_space_size);
...
#pragma omp parallel for
    ...
    for (int t = (seq_length - 1); t >= 0; t--) {
        for (int l = (num_layers - 1); l >= 0; l--) {
            if (t == 0) {
                transpose<data_t>(cx_ptr + l * h_size,
                    ts_ + temp_space_off,
                    batch_size, state_size);
            }
            ...
            for(int h = 0; h < h_size; h++) {
                it = ws_ptr[gates_space_off + l * gates_size + t * gates_nlayer_size + h];
                ft = ws_ptr[gates_space_off + l * gates_size + t * gates_nlayer_size + h_size + h];
                ot = ws_ptr[gates_space_off + l * gates_size + t * gates_nlayer_size + 2 * h_size + h];
                gt = ws_ptr[gates_space_off + l * gates_size + t * gates_nlayer_size + 3 * h_size + h];
                if (t == 0)
                ...
                ts_[dgates_space_off + l * gates_size + t*gates_nlayer_size + h] = dit;
                ts_[dgates_space_off + l * gates_size + t*gates_nlayer_size + h_size + h] = dft;
                ts_[dgates_space_off + l * gates_size + t*gates_nlayer_size + 2 * h_size + h] = dot;
                ts_[dgates_space_off + l * gates_size + t*gates_nlayer_size + 3 * h_size + h] = dgt;
           }
           if (t == 0) {
            ...
            if ((t > 0) && (l > 0)) {
                ...
                transpose<data_t>(x_ptr + t * x_size,
                    ts_ + temp_space_off,
                ...
                transpose<data_t>(x_ptr,
                    ts_ + temp_space_off,
                    batch_size, input_size);
                transpose<data_t>(hx_ptr,
                    ts_ + temp_space_off + x_size,
                    batch_size, state_size);               
                array_set(ts_ + temp_space_off
                      + x_size + h_size,
            ...
            gemm<data_t>(NOTRANS, TRANS, ts_ + temp_space_off,
                ...
                dweights_ptr + woffset,
                in_size + state_size + 2, 4 * state_size, batch_size, 1);

            gemm<data_t>(NOTRANS, NOTRANS, weights_ptr + woffset,
                ts_ + dgates_space_off
                    + l * gates_size + t * gates_nlayer_size,
                ts_ + temp_space_off,
                in_size + state_size + 2, batch_size, 4 * state_size, 0);
            ...
            if ((t > 0) && (l > 0)) {
                ...
                transpose<data_t>(ts_ + temp_space_off, dx_ptr,
                    input_size, batch_size);
...
template <typename data_t>
class lstm_backward_test : public ::testing::TestWithParam<lstm_bwd_test_params> {
protected:
    virtual void SetUp()
    {
        using namespace mkldnn::impl::utils;
        lstm_bwd_test_params p
                = ::testing::TestWithParam<lstm_bwd_test_params>::GetParam();
        ...
        fill_data<data_t>(l_dweights.get_primitive_desc().get_size() / sizeof(data_t),
                (data_t *)l_dweights.get_data_handle());
        fill_data<data_t>(l_ref_dweights.get_primitive_desc().get_size() / sizeof(data_t),
                (data_t *)l_ref_dweights.get_data_handle());       

        // Execute
        std::vector<primitive> pipeline;
        auto s = stream(stream::kind::lazy);
        auto l = rnn_backward(rnn_prim_desc, l_x, l_hx, l_cx, l_dy, l_dhy, l_dcy, l_weights, l_ws, l_dx, l_dhx, l_dcx, l_dweights);
        pipeline.push_back(l);
        s.submit(pipeline).wait();
        
        compute_ref_lstm_bwd<data_t>(ld, l_x_desc, l_hx_desc, l_y_desc, l_weights_desc,
            l_x, l_hx, l_cx, l_dy, l_dhy, l_dcy, l_weights, l_ws, l_ref_dx, l_ref_dhx, l_ref_dcx, l_ref_dweights);
...
using lstm_bwd_test_params_float = lstm_bwd_test_params;
