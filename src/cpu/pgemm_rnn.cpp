...
inline void lstm_fwd_prop_single(const size_t input_size,
    ...
    if (tranx == TRANS) {
        omatcopy<data_trait<data_t>::data_type>('R', 'T', batch_size,
    ...
    if (tranht_1 == TRANS) {
        ...
        cblas_copy<data_trait<data_t>::data_type>(
...
inline void lstm_bwd_prop_single(const size_t input_size,
    ...
    array_set(tmp + x_size + h_size, 1.0, 2 * batch_size);
    cblas_gemm<data_trait<data_t>::data_type>(CblasRowMajor, CblasNoTrans,
    ...
    for (int l = 0; l < total_layers; l++) {
        ...
        if (dl == 0) {
            for (int t = 0; t < seq_length; t++) {
                ...
                // save y output
                if (rl == (num_layers - 1)) {
                    omatcopy<data_trait<data_t>::data_type>('R', 'T',
...
#pragma omp parallel for
        for (size_t ly = 0; ly < num_layers; ly++) {
            ...
            cblas_axpy<data_trait<data_t>::data_type>(h_size, 1.0,
...
void pgemm_rnn_fwd_t<data_type>::execute_forward() {
    ...
    lstm_fwd_prop(seq_length, num_layers, batch_size, input_size, state_size,
            direction, w1_size, wx_size, h_size, x_size, h_nlayer_size,
            gates_size, gates_nlayer_size, gates_space_size, h_space_size, x,
            hx, cx, w, y, hy, cy, ws, ts_);
#endif // USE_MKL
}
...
void pgemm_rnn_bwd_t<data_type>::execute_backward() {
    ...
    const size_t gates_space_size = conf_.gates_space_size();
    const size_t h_space_size = conf_.h_space_size();
    lstm_bwd_prop(seq_length, num_layers, batch_size, input_size, state_size,
            direction, w1_size, wx_size, h_size, x_size, h_nlayer_size,
            gates_size, gates_nlayer_size, gates_space_size, h_space_size,
            state_outputs, x, hx, cx, dy, dhy, dcy, w, ws, dx, dhx, dcx, dw,
            ts_);
#endif // USE_MKL
}
